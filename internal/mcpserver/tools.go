...
func Tools() []server.ServerTool {
	return []server.ServerTool{
		...
		{Tool: SignCertificateTool, Handler: HandleSignCertificate},
		{Tool: GenerateIntermediateCATool, Handler: HandleGenerateIntermediateCA},
		{Tool: GenerateCRLTool, Handler: HandleGenerateCRL},
		{Tool: ParseCRLTool, Handler: HandleParseCRL},
		{Tool: VerifyCRLSignatureTool, Handler: HandleVerifyCRLSignature},
		{Tool: CheckCertRevokedByCRLTool, Handler: HandleCheckCertRevokedByCRL},
		{Tool: CloneCertificateTool, Handler: HandleCloneCertificate},
		{Tool: GenerateDomainVariantsTool, Handler: HandleGenerateDomainVariants},
		{Tool: FingerprintMatchTool, Handler: HandleFingerprintMatch},
		{Tool: FingerprintMatchByHashTool, Handler: HandleFingerprintMatchByHash},
		{Tool: DetectChangeTool, Handler: HandleDetectChange},
		{Tool: CertMapScanTool, Handler: HandleCertMapScan},
		{Tool: CertMapParseFilesTool, Handler: HandleCertMapParseFiles},
		{Tool: CertMapTimelineTool, Handler: HandleCertMapTimeline},
	}
}
...
var SignCertificateTool = mcp.NewTool("cert_sign_certificate",
	mcp.WithDescription(
		"Sign a terminal/leaf certificate using a CA certificate and private key. "+
			"Creates a server, client, or dual-purpose certificate signed by the specified CA. "+
			"Supports RSA, ECDSA, and Ed25519 keys. The CA must have CertSign key usage."),
	mcp.WithString("ca_cert_path",
		mcp.Required(),
		mcp.Description("Path to the CA certificate PEM file"),
	),
	mcp.WithString("ca_key_path",
		mcp.Required(),
		mcp.Description("Path to the CA private key PEM file"),
	),
	mcp.WithString("common_name",
		mcp.Description("Common Name (CN) for the certificate. Default: 'localhost'"),
	),
	mcp.WithString("organization",
		mcp.Description("Organization name. Default: inherits from CA certificate"),
	),
	mcp.WithString("country",
		mcp.Description("Two-letter country code. Default: inherits from CA certificate"),
	),
	mcp.WithString("province",
		mcp.Description("Province or state. Default: inherits from CA certificate"),
	),
	mcp.WithString("locality",
		mcp.Description("City or locality. Default: inherits from CA certificate"),
	),
	mcp.WithArray("dns_names",
		mcp.Description("Subject Alternative Names - additional DNS names (e.g., ['www.example.com', 'api.example.com'])"),
	),
	mcp.WithArray("ip_addresses",
		mcp.Description("Subject Alternative Names - IP addresses as strings (e.g., ['192.168.1.1'])"),
	),
	mcp.WithNumber("validity_days",
		mcp.Description("Certificate validity period in days. Default: 365"),
	),
	mcp.WithNumber("key_size",
		mcp.Description("Key size in bits. RSA: 2048 (default) or 4096. ECDSA: 256 (P-256), 384 (P-384), 521 (P-521)."),
	),
	mcp.WithString("key_type",
		mcp.Description("Key algorithm type. Options: 'rsa' (default), 'ecdsa', 'ed25519'"),
	),
	mcp.WithString("key_usage",
		mcp.Description("Certificate key usage. Options: 'server' (default), 'client', 'both'"),
	),
	mcp.WithString("output_cert_path",
		mcp.Description("File path for the generated certificate. Default: '<common_name>.pem'"),
	),
	mcp.WithString("output_key_path",
		mcp.Description("File path for the generated private key. Default: '<common_name>-key.pem'"),
	),
)

var GenerateIntermediateCATool = mcp.NewTool("cert_generate_intermediate_ca",
	mcp.WithDescription(
		"Generate an intermediate CA certificate signed by a parent/root CA. "+
			"The intermediate CA can sign end-entity certificates, creating a multi-tier PKI hierarchy. "+
			"Supports RSA 4096-bit (default), ECDSA P-384, and Ed25519 keys. "+
			"The parent CA must have CertSign key usage."),
	mcp.WithString("parent_cert_path",
		mcp.Required(),
		mcp.Description("Path to the parent/root CA certificate PEM file"),
	),
	mcp.WithString("parent_key_path",
		mcp.Required(),
		mcp.Description("Path to the parent/root CA private key PEM file"),
	),
	mcp.WithString("common_name",
		mcp.Description("Common Name (CN) for the intermediate CA. Default: 'Intermediate CA'"),
	),
	mcp.WithString("organization",
		mcp.Description("Organization name. Default: inherits from parent CA"),
	),
	mcp.WithString("country",
		mcp.Description("Two-letter country code. Default: inherits from parent CA"),
	),
	mcp.WithString("province",
		mcp.Description("Province or state. Default: inherits from parent CA"),
	),
	mcp.WithString("locality",
		mcp.Description("City or locality. Default: inherits from parent CA"),
	),
	mcp.WithNumber("validity_days",
		mcp.Description("Certificate validity period in days. Default: 1825 (5 years)"),
	),
	mcp.WithNumber("key_size",
		mcp.Description("Key size in bits. RSA: 4096 (default), ECDSA: 384 (P-384)"),
	),
	mcp.WithString("key_type",
		mcp.Description("Key algorithm type. Options: 'rsa' (default), 'ecdsa', 'ed25519'"),
	),
	mcp.WithNumber("path_len_constraint",
		mcp.Description("Path length constraint. 0 = no further intermediate CAs allowed, -1 = unlimited. Default: 0"),
	),
	mcp.WithString("output_cert_path",
		mcp.Description("File path for the generated intermediate CA certificate"),
	),
	mcp.WithString("output_key_path",
		mcp.Description("File path for the generated intermediate CA private key"),
	),
)

var GenerateCRLTool = mcp.NewTool("cert_generate_crl",
	mcp.WithDescription(
		"Generate a Certificate Revocation List (CRL) signed by a CA. "+
			"Lists revoked certificates by serial number with revocation dates and RFC 5280 reason codes. "+
			"The CA must have CRLSign key usage."),
	mcp.WithString("ca_cert_path",
		mcp.Required(),
		mcp.Description("Path to the CA certificate PEM file"),
	),
	...
	mcp.WithArray("serial_numbers",
		mcp.Description("List of revoked certificate serial numbers (e.g., ['123456', '789012'])"),
	),
	mcp.WithArray("reasons",
		mcp.Description("List of revocation reasons corresponding to serial numbers. Options: 'unspecified', 'key-compromise', 'ca-compromise', 'affiliation-changed', 'superseded', 'cessation-of-operation', 'certificate-hold', 'privilege-withdrawn', 'aa-compromise'"),
	),
	mcp.WithNumber("next_update_days",
		mcp.Description("Next update period in days. Default: 30"),
	),
	mcp.WithNumber("crl_number",
		mcp.Description("CRL sequence number. Default: 1"),
	),
	mcp.WithString("output_path",
		mcp.Description("File path for the generated CRL. Default: 'crl.pem'"),
	),
)

var ParseCRLTool = mcp.NewTool("cert_parse_crl",
	mcp.WithDescription(
		"Parse a Certificate Revocation List (CRL) file and return detailed information "+
			"including issuer, this/next update times, CRL number, signature algorithm, "+
			"and list of revoked certificates with serial numbers, revocation dates, and reasons."),
	mcp.WithString("crl_path",
		mcp.Required(),
		mcp.Description("Path to the CRL file (PEM or DER format)"),
	),
)

var VerifyCRLSignatureTool = mcp.NewTool("cert_verify_crl_signature",
	mcp.WithDescription(
		"Verify a CRL's signature against a CA certificate. Checks that the CRL "+
			"was actually signed by the claimed CA. Returns verification status with details."),
	mcp.WithString("crl_path",
		mcp.Required(),
		mcp.Description("Path to the CRL file"),
	),
	mcp.WithString("ca_cert_path",
		mcp.Required(),
		mcp.Description("Path to the CA certificate PEM file to verify against"),
	),
)

var CheckCertRevokedByCRLTool = mcp.NewTool("cert_check_revoked_by_crl",
	mcp.WithDescription(
		"Check if a certificate is revoked in a specific CRL. Compares the certificate's "+
			"serial number against the CRL's revoked certificate list."),
	mcp.WithString("cert_path",
		mcp.Required(),
		mcp.Description("Path to the certificate PEM file to check"),
	),
	mcp.WithString("crl_path",
		mcp.Required(),
		mcp.Description("Path to the CRL file to check against"),
	),
)

var CloneCertificateTool = mcp.NewTool("cert_clone_certificate",
	mcp.WithDescription(
		"Clone an existing certificate by copying its subject information and extensions, "+
			"but generating a new key pair and serial number. The cloned certificate will have "+
			"different fingerprints. For authorized security testing only."),
	mcp.WithString("source_cert_path",
		mcp.Required(),
		mcp.Description("Path to the source certificate PEM file to clone"),
	),
	mcp.WithNumber("key_size",
		mcp.Description("New key size in bits. Default: 2048"),
	),
	mcp.WithString("key_type",
		mcp.Description("New key algorithm type. Options: 'rsa' (default), 'ecdsa', 'ed25519'"),
	),
	mcp.WithNumber("validity_days",
		mcp.Description("New certificate validity period in days. Default: 365"),
	),
	mcp.WithBoolean("modify_subject",
		mcp.Description("Whether to modify the subject information. Default: false"),
	),
	mcp.WithString("new_common_name",
		mcp.Description("New Common Name (CN) for the cloned cert (requires modify_subject=true)"),
	),
	mcp.WithString("new_organization",
		mcp.Description("New organization name for the cloned cert (requires modify_subject=true)"),
	),
	mcp.WithString("ca_cert_path",
		mcp.Description("Optional CA certificate path for CA-signed clone (instead of self-signed)"),
	),
	mcp.WithString("ca_key_path",
		mcp.Description("Optional CA private key path for CA-signed clone"),
	),
	mcp.WithString("output_cert_path",
		mcp.Description("File path for the cloned certificate"),
	),
	mcp.WithString("output_key_path",
		mcp.Description("File path for the cloned private key"),
	),
)

var GenerateDomainVariantsTool = mcp.NewTool("cert_generate_domain_variants",
	mcp.WithDescription(
		"Generate certificates for domain variants including homoglyphs (visual character substitution), "+
			"subdomains, TLD changes, hyphenation, and character insertion. Useful for phishing detection "+
			"and typosquatting research. For authorized security testing only. Max 50 variants."),
	mcp.WithString("base_domain",
		mcp.Required(),
		mcp.Description("Base domain name to generate variants for (e.g., 'example.com')"),
	),
	mcp.WithArray("variant_types",
		mcp.Description("Variant types to generate. Options: ['homoglyph', 'subdomain', 'tld', 'hyphenation', 'insertion']. Default: all types"),
	),
	mcp.WithNumber("key_size",
		mcp.Description("Key size in bits. Default: 2048"),
	),
	mcp.WithString("key_type",
		mcp.Description("Key algorithm type. Options: 'rsa' (default), 'ecdsa', 'ed25519'"),
	),
	...
	mcp.WithString("output_dir",
		mcp.Description("Directory to save generated certificate files. Default: current directory"),
	),
	mcp.WithString("organization",
		mcp.Description("Organization name for the generated certificates"),
	),
	mcp.WithString("ca_cert_path",
		mcp.Description("Optional CA certificate path for CA-signed variants"),
	),
	mcp.WithString("ca_key_path",
		mcp.Description("Optional CA private key path for CA-signed variants"),
	),
)

// FingerprintMatchTool matches TLS fingerprints against known services.
var FingerprintMatchTool = mcp.NewTool("cert_match_fingerprints",
	mcp.WithDescription("Match TLS fingerprints (JARM, JA3, cert) against a database of known services for identification and C2 detection"),
	mcp.WithString("target",
		mcp.Required(),
		mcp.Description("Domain name or IP address with optional port (e.g., 'example.com:8443'). Default port is 443."),
	),
...
// FingerprintMatchByHashTool matches a single fingerprint hash.
var FingerprintMatchByHashTool = mcp.NewTool("cert_match_fingerprint_by_hash",
	mcp.WithDescription("Match a single fingerprint hash against the built-in database of known services"),
	mcp.WithString("type",
		mcp.Required(),
		mcp.Description("Fingerprint type: jarm, ja3, cert_sha256, spki"),
	),
	mcp.WithString("hash",
		mcp.Required(),
		mcp.Description("Fingerprint hash to match (hex, with or without colons)"),
	),
)

// DetectChangeTool detects certificate changes from a previous snapshot.
var DetectChangeTool = mcp.NewTool("cert_detect_change",
	mcp.WithDescription("Detect certificate changes by comparing current state against a previous snapshot. "+
		"Detects certificate renewal, key rotation, issuer changes, JARM changes, and expiry. "+
		"Essential for continuous monitoring in cyberspace mapping."),
	mcp.WithString("target",
		mcp.Required(),
		mcp.Description("Domain name or IP address with optional port (e.g. 'example.com:8443')"),
	),
	mcp.WithString("snapshot_dir",
		mcp.Description("Directory to store snapshots (default: in-memory, not persisted)"),
	),
	mcp.WithBoolean("save",
		mcp.Description("Save the new snapshot to disk after comparison (requires snapshot_dir)"),
	),
)
