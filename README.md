...
- When local skill eval, benchmark, blind-comparison, or description-optimization outputs are present, validation also checks official generated-output placement and field shapes such as `eval_metadata.json`, `grading.json`, `metrics.json`, `timing.json`, `history.json`, `benchmark.json`, `feedback.json`, `comparison-N.json`, `analysis.json`, and trigger eval sets, including nested grading, benchmark result, benchmark notes, benchmark summary stat, comparison, analysis, and A/B blind-comparison label fields.
...
- 如果本地存在 skill eval、benchmark、blind-comparison 或 description-optimization 输出，校验还会检查官方 generated-output 位置和字段结构，例如 `eval_metadata.json`、`grading.json`、`metrics.json`、`timing.json`、`history.json`、`benchmark.json`、`feedback.json`、`comparison-N.json`、`analysis.json` 和 trigger eval sets，包括 grading、benchmark result、benchmark notes、benchmark summary stat、comparison、analysis 与 A/B blind-comparison 标签字段。
