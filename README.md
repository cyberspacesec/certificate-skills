...
- When local skill eval or benchmark outputs are present, validation also checks viewer-facing `grading.json`, `benchmark.json`, and `feedback.json` field shapes.
...
- 如果本地存在 skill eval 或 benchmark 输出，校验还会检查 viewer-facing `grading.json`、`benchmark.json` 和 `feedback.json` 字段结构。
