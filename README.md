...
- When local skill eval or benchmark outputs are present, validation also checks official output field shapes such as `eval_metadata.json`, `grading.json`, `metrics.json`, `timing.json`, `history.json`, `benchmark.json`, and `feedback.json`.
...
- 如果本地存在 skill eval 或 benchmark 输出，校验还会检查官方输出字段结构，例如 `eval_metadata.json`、`grading.json`、`metrics.json`、`timing.json`、`history.json`、`benchmark.json` 和 `feedback.json`。
