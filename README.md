...
- Generated `.skill` archives, skill eval workspaces, and `dist/` outputs are ignored and must not be tracked as source files.
...
- 生成的 `.skill` archives、skill eval workspaces 和 `dist/` 输出会被忽略，不能作为源码跟踪。
