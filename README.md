...
- When local skill eval, benchmark, or blind-comparison outputs are present, validation also checks official output field shapes such as `eval_metadata.json`, `grading.json`, `metrics.json`, `timing.json`, `history.json`, `benchmark.json`, `feedback.json`, `comparison-N.json`, and `analysis.json`.
...
- 如果本地存在 skill eval、benchmark 或 blind-comparison 输出，校验还会检查官方输出字段结构，例如 `eval_metadata.json`、`grading.json`、`metrics.json`、`timing.json`、`history.json`、`benchmark.json`、`feedback.json`、`comparison-N.json` 和 `analysis.json`。
